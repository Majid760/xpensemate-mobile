...
class BudgetGoalsInsightModel extends BudgetGoalsInsightEntity {
  ...
  factory BudgetGoalsInsightModel.fromJson(Map<String, dynamic> json) {
    try {
      ...
      late List<BudgetGoalModel> allGoals = <BudgetGoalModel>[];
      ...
      return BudgetGoalsInsightModel(
        ...
        achievedGoals: _parseBudgetGoalsList(data['achievedGoals'] as List? ?? []),
        failedGoals: _parseBudgetGoalsList(data['failedGoals'] as List? ?? []),
        terminatedGoals: _parseBudgetGoalsList(data['terminatedGoals'] as List? ?? []),
        totalBudgeted: (data['totalBudgeted'] as num?)?.toDouble() ?? 0.0,
        avgProgress: (data['avgProgress'] as num?)?.toDouble() ?? 0.0,
        closestGoals: _parseBudgetGoalsList(data['closestGoals'] as List? ?? []),
        overdueGoals: _parseBudgetGoalsList(data['overdueGoals'] as List? ?? []),
      );
    } catch (e) {
  ...
  factory BudgetGoalsInsightModel.fromEntity(BudgetGoalsInsightEntity entity) => BudgetGoalsInsightModel(
        totalGoals: entity.totalGoals,
        activeGoals: entity.activeGoals.map(BudgetGoalModel.fromEntity).toList(),
        achievedGoals: entity.achievedGoals.map(BudgetGoalModel.fromEntity).toList(),
        failedGoals: entity.failedGoals.map(BudgetGoalModel.fromEntity).toList(),
        terminatedGoals: entity.terminatedGoals.map(BudgetGoalModel.fromEntity).toList(),
        totalBudgeted: entity.totalBudgeted,
        avgProgress: entity.avgProgress,
        closestGoals: entity.closestGoals.map(BudgetGoalModel.fromEntity).toList(),
        overdueGoals: entity.overdueGoals.map(BudgetGoalModel.fromEntity).toList(),
      );

  BudgetGoalsInsightEntity toEntity() => BudgetGoalsInsightEntity(
        totalGoals: totalGoals,
        activeGoals: activeGoals.map((model) => (model as BudgetGoalModel).toEntity()).toList(),
        achievedGoals: achievedGoals.map((model) => (model as BudgetGoalModel).toEntity()).toList(),
        failedGoals: failedGoals.map((model) => (model as BudgetGoalModel).toEntity()).toList(),
        terminatedGoals: terminatedGoals.map((model) => (model as BudgetGoalModel).toEntity()).toList(),
        totalBudgeted: totalBudgeted,
        avgProgress: avgProgress,
        closestGoals: closestGoals.map((model) => (model as BudgetGoalModel).toEntity()).toList(),
        overdueGoals: overdueGoals.map((model) => (model as BudgetGoalModel).toEntity()).toList(),
      );

  Map<String, dynamic> toJson() => {
        'totalGoals': totalGoals,
        'activeGoals': activeGoals.map((e) => (e as BudgetGoalModel).toJson()).toList(),
        'achievedGoals': achievedGoals.map((e) => (e as BudgetGoalModel).toJson()).toList(),
        'failedGoals': failedGoals.map((e) => (e as BudgetGoalModel).toJson()).toList(),
        'terminatedGoals': terminatedGoals.map((e) => (e as BudgetGoalModel).toJson()).toList(),
        'totalBudgeted': totalBudgeted,
        'avgProgress': avgProgress,
        'closestGoals': closestGoals.map((e) => (e as BudgetGoalModel).toJson()).toList(),
        'overdueGoals': overdueGoals.map((e) => (e as BudgetGoalModel).toJson()).toList(),
      };
